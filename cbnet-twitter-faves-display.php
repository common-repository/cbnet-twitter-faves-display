<?php /* widget_cbnet_plugin_twitterfaves */
/**
 * Plugin Name: cbnet Twitter Faves Display
 * Plugin URI: http://www.chipbennett.net/wordpress/plugins/cbnet-twitter-faves-display/
 * Description: A widget that displays tweets from any user's Twitter faves.
 * Version: 1.0
 * Author: chipbennett
 ...
 * @since 1.0
 */
add_action( 'admin_notices', 'cbnet_twitter_faves_admin_notice' );

function cbnet_twitter_faves_admin_notice() {
	if ( ! file_exists( CBNETTWITTERWIDGETPATH ) ) {
		echo "<div class='updated' style='background-color:#f66;'><p>" . sprintf( 'The cbnet Twitter Faves Display Plugin is obsolete and no longer maintained. <strong>Please <a href="%s">install the cbnet Twitter Widget Plugin</a> in order to continue to receive updates and support.</strong>', cbnet_twitter_plugin_install_path() ) . "</p></div>";
...
/**
 * widget_cbnet_plugin_twitterfaves class.
...
class widget_cbnet_plugin_twitterfaves extends WP_Widget {

    function widget_cbnet_plugin_twitterfaves() {
        $widget_ops = array('classname' => 'widget-cbnet-plugin-twitterfaves', 'description' => 'cbnet plugin widget to display Twitter favorites tweets' );
        $this->WP_Widget('cbnet_plugin_twitterfaves', 'cbnet Twitter Faves', $widget_ops);
    }

    function widget( $args, $instance ) {
        extract($args);
        $title = apply_filters('widget_title', empty($instance['title']) ? 'Twitter Faves' : $instance['title']);
		$twitteruserid =  $instance['twitteruserid'];
		$favestitle = $instance['favestitle'];
		$favesdesc = $instance['favesdesc'];
...
new TWTR.Widget({
  ...
  title: '<?php echo $favestitle; ?>',
  subject: '<?php echo $favesdesc; ?>',
...
}).render().setUser('<?php echo $twitteruserid; ?>').start();
</script>
<br />
<!-- End Twitter List -->
...
<?php
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['favestitle'] = strip_tags($new_instance['favestitle']);
        $instance['favesdesc'] = strip_tags($new_instance['favesdesc']);
...
<p>
<label for="<?php echo $this->get_field_id('favestitle'); ?>">Title:</label> 
<input class="widefat" id="<?php echo $this->get_field_id('favestitle'); ?>" name="<?php echo $this->get_field_name('favestitle'); ?>" type="text" value="<?php echo $instance['favestitle']; ?>" />
</p>
<p>
<label for="<?php echo $this->get_field_id('favesdesc'); ?>">Caption:</label> 
<input class="widefat" id="<?php echo $this->get_field_id('favesdesc'); ?>" name="<?php echo $this->get_field_name('favesdesc'); ?>" type="text" value="<?php echo $instance['favesdesc']; ?>" />
</p>
<p>
